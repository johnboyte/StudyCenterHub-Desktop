...
func _init() -> void:
	...
	var db_path = ProjectSettings.globalize_path("user://test_response_center.db")
	...
	var vm = null
	for v in vms:
		if v["item_uuid"] == vm_uuid:
			vm = v
			break
			
	if vm == null or vm["matched_caller_name"] != "Samantha Diaz":
		print("FAIL: Caller ID phone matching failed. Expected Samantha Diaz, got: ", vm["matched_caller_name"] if vm else "null")
		quit(1)
		return
	...
	if check_vm == null or check_vm["status"] != "in_progress" or check_vm["priority"] != "High" or int(check_vm["assigned_person_id"]) != 1 or check_vm["assignee_name"] != "Marcus Vance":

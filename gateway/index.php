...
// Route: TTS Voice Preview Test Call Endpoint (authenticated server-side request to Twilio)
if ($uri === '/api/v1/tts/preview-call') {
    header('Content-Type: application/json; charset=utf-8');
    $req_key = $_SERVER['HTTP_X_SYNC_API_KEY'] ?? $_GET['sync_api_key'] ?? $_POST['sync_api_key'] ?? '';
    if ($req_key !== $sync_api_key) {
        http_response_code(401);
        echo json_encode(['success' => false, 'error' => 'Unauthorized']);
        exit;
    ...
    $voice = trim($input['voice'] ?? $_POST['voice'] ?? $_GET['voice'] ?? '');
    $text = trim($input['text'] ?? $_POST['text'] ?? $_GET['text'] ?? '');
    $to_phone = trim($input['to'] ?? $input['to_phone'] ?? $input['destination'] ?? $_POST['to'] ?? $_POST['to_phone'] ?? $_GET['to'] ?? '');
    ...
    if (!in_array($voice, $allowed_voices, true)) {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'Invalid or unapproved voice identifier: ' . $voice]);
        exit;
    }

    if (empty($to_phone)) {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'Missing destination phone number']);
        exit;
    }

    if (empty($text)) {
        $text = "This is a live preview call from Real Life Study Center Hub using " . str_replace('Polly.', '', $voice) . ".";
    }
    ...
    if ($http_code >= 200 && $http_code < 300 && !empty($json['sid'])) {
        echo json_encode([
            ...
            'message' => 'Preview call initiated via Twilio'
        ]);
    } else {
        http_response_code($http_code ?: 500);
        echo json_encode([
            'success' => false,
            'error' => $json['message'] ?? 'Twilio API call failed',
            'http_code' => $http_code
        ]);
    }
    exit;

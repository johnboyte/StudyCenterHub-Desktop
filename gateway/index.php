...
if (($uri === '/api/v1/public/sessions' || $uri === '/public/api/sessions') && ($method === 'GET' || $method === 'POST')) {
    ...
    $member_signups = [];
    if (!empty($e164)) {
        $m_stmt = $pdo->prepare("SELECT session_id, signup_status FROM member_signups_index WHERE phone_e164 = ?");
        $m_stmt->execute([$e164]);
...
if (($uri === '/api/v1/public/checkin' || $uri === '/public/api/checkin') && $method === 'POST') {
    ...
    $payload['event_uuid'] = $event_uuid;
    $payload['checkInDate'] = gmdate('Y-m-d');
    $payload['received_at'] = gmdate('Y-m-d H:i:s') . ' UTC';
...
<body>
    ...
    <main class="app-container">
        ...
        <section id="screenCheckInNotice" class="card hidden">
            ...
            <div id="checkinAlreadyView" class="hidden">
                <div class="success-hero">
                    <div class="success-icon">😊</div>
                    <h2 class="page-title" id="checkinAlreadyTitle">You’re Already Checked In Today!</h2>
    ...
    <script>
        ...
        function getActiveMemberIdentity() {
            if (matchedMemberData && matchedMemberData.phone) {
        ...
        function pollCheckInStatus(eventUuid, firstName, pollCount) {
            ...
            .then(data => {
                document.getElementById('checkinProcessingView').classList.add('hidden');
                if (data.status === 'checked_in' || data.status === 'processed' || data.status === 'success' || data.status === 'ok') {
                    ...
                    document.getElementById('checkinAlreadyTitle').textContent = '✓ You’re Already Checked In Today!';
                    document.getElementById('checkinAlreadyMsg').textContent = 'Glad you’re here, ' + (data.first_name || firstName) + '!';
                    ...
                    if (pollCount < maxPolls) {
                        ...
                        document.getElementById('checkinSuccessMsg').textContent = 'Your check-in has been submitted to StudyCenterHub. Welcome, ' + firstName + '!';
                        document.getElementById('checkinSuccessView').classList.remove('hidden');
                    }
                } else {
                    showCheckInError(data.message || 'Check-in processing error');
                }
            })
            .catch(e => {
                document.getElementById('checkinProcessingView').classList.add('hidden');
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (data.event_uuid) {
                        ...
                        showRegistrationSuccess(regData.firstName, true);
                    }
                } else {
            ...
            .then(data => {
                ...
                if (st === 'registered_and_checked_in' || st === 'registered_already_checked_in' || st === 'registered') {
                    ...
                    if (pollCount < maxPolls) {
                        ...
                        showRegistrationSuccess(firstName, true);
                    }
                } else {
                    showRegistrationSuccess(firstName, true);
                }

...
func _populate_team_leaders() -> void:
	...
	var p_res = db.execute("SELECT first_name, last_name FROM people WHERE is_team_leader_eligible = 1 AND (status IS NULL OR status = 'active' OR status = '') ORDER BY last_name ASC, first_name ASC;")
